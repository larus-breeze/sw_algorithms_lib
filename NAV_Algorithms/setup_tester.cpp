...
//  sensor_q.from_euler( 10.0 * M_PI/180.0, -75.0 * M_PI/180.0, +125.0 * M_PI/180.0);
  ...
  float sensor_orientation_pitch = 20.0f * M_PI_F / 180;
  float sensor_orientation_yaw   = -30.0f * M_PI_F / 180;

  float pitch_angle_delta = 1.5  * M_PI_F / 180; 	// sensor nose is higher than configured
  float roll_angle_delta  = -1.5 * M_PI_F / 180; 	// sensor is looking more left than configured
...
#else // also use unity_vector_down_body[RIGHT] ?
